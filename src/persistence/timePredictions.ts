...
export async function getTimePredictions():Promise<TimePredictions|null> {
  ...
  try {
    ...
    if (!_isTimePredictions(predictions)) { console.error(`Persisted value at ${TIME_PREDICTIONS_KEY} is in unexpected format.`); return null; }
    return predictions;
  } catch(e) {
    console.error(`Failed to parse time predictions. ${e}`);
    return null;
  }
}
